        ...
        **kwargs: Any,
        ...
        if (
            ...
            if probe_hits:
                ...
                payload = {
                    "query": query,
                    "scope": "session" if active_session_key is not None else "none",
                    "session_key": active_session_key,
                    "requested_scope": "all",
                    "total": 0,
                    "limit": limit_int,
                    ...
                    "hits": [],
                    "note": (
                        "Broad scope='all' query was not searched because the active "
                        "session had no matching evidence and the query did not contain "
                        "a stable anchor such as an id, address, tx hash, filename, "
                        "or exact tool output fragment."
                    ),
                }
                if default_assistant_filter_active:
                    payload["omitted_assistant_replies"] = 0
                return json.dumps(payload, ensure_ascii=False, default=str)
