    ...
    async def _do_reload_skills(self) -> str:
        ...
        if added:
            ...
            parts.append(
                ...
                "The newest user request selects the workflow and count; "
                "SKILL.md after-action, continuous, recovery, lifecycle, "
                "settlement, and post-action rules define how to finish each "
                "selected workflow unit through its contract-defined terminal "
                "outcome or concrete blocker. Do not use those rules to start "
                "another repeated unit, larger count, or unrelated side effect "
                "unless the newest request explicitly asks for that larger "
                "scope; continuation-only messages authorize at most one "
                "bounded unit. "
