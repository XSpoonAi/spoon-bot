...
_EXTERNAL_SIDE_EFFECT_BOUNDARY = (
    ...
    "selected; use the active SKILL.md contract to choose steps and completion "
    "criteria inside that requested workflow unit. If the contract says an "
    "after-action, continuous, lifecycle, recovery, or settlement step is "
    "required to finish the selected unit, continue through that unit until "
    "its contract-defined terminal outcome or concrete blocker. Do not use "
    "that lifecycle guidance as permission to start another repeated unit, "
    "larger count, or unrelated side effect unless the newest user request "
    "explicitly selected that larger scope. If the newest request gives an "
    "ordered workflow, complete that workflow in order until its real terminal "
    "outcome or concrete blocker. Do not branch into alternative external "
    "actions, restart the same completed side effect, or repeat a paid "
...
_BOUNDED_CONTINUATION_BOUNDARY = (
    ...
    "short continuation of earlier work, it authorizes at most one bounded "
    "workflow unit from the immediately preceding selected workflow. If same-"
    "session evidence shows that unit is unfinished, finish that unit through "
    "its documented terminal outcome or concrete blocker. If the prior unit is "
    "complete and the same workflow has a clear next single-unit action, a "
    "continuation-only message can start at most one new unit. Use prior "
    "request facts as checkpoints, not renewed permission for older counts, "
    "multi-run targets, or all remaining repetitions. Prior user request text "
    "is intentionally not repeated for plain continuations; use assistant/tool "
    "state evidence and live state instead. If the evidence is stale, read-only, "
    "or ambiguous, ask a concise clarification or report current status instead "
