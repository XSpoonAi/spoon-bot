...
**No-progress wait boundary**: Do not poll an external system indefinitely. If repeated checks show the same pending/waiting/not-ready state without material progress, report the current blocker and what evidence would let the user resume later. Continue automatically only while each check produces new progress or is still within a bounded wait required by the newest request.
...
- If the remaining step depends on an external state change and repeated checks show no material progress, stop with the blocker instead of continuing to wait, poll, or ask for generic feedback.
