...
from spoon_bot.agent.tools.execution_context import get_tool_workspace
...
def _get_workspace() -> Path:
    ...
    if tool_workspace := get_tool_workspace():
        return Path(tool_workspace)
    module_path = Path(__file__).resolve()
    if module_path.parent.parent.name == "skills":
        ...
        # ----------------------------------------------------------
            ...
            try:
                ...
                return (
                    ...
                    "then follow the commands and persistence rules described in SKILL.md. "
                    "This install replaced the skill directory; dependency/setup evidence "
                    "from before this install is stale."
                )
        ...
        # ----------------------------------------------------------
            ...
            try:
                ...
                return (
                    ...
                    f"2) read_file(path='{_rel_path}/SKILL.md') for instructions.\n"
                    "3) Complete any Setup/Prerequisites/Install/dependency steps first, "
                    "then execute CLI commands from SKILL.md. This install replaced the "
                    "skill directory; dependency/setup evidence from before this install "
                    "is stale."
                )
            except Exception as e:
        ...
        # ----------------------------------------------------------
            ...
            try:
                ...
                return (
                    ...
                    "primary commands, then record the update in soul.md. This update "
                    "replaced the skill directory; dependency/setup evidence from before "
                    "this update is stale."
                )
            except Exception as e:
