...
def generate_markdown(out_path: Path) -> str:
    ...
    w()
    w("Base URL (local): `http://localhost:16600`  ")
    w("API Docs (Swagger): `http://localhost:16600/docs`  ")
    w("API Docs (ReDoc): `http://localhost:16600/redoc`")
    w()
    ...
    w("### Connection")
    w()
    w("**Endpoint:** `ws://localhost:16600/v1/ws`")
    w()
    w("**Authentication via Query Parameters:**")
    w("```")
    w("ws://localhost:16600/v1/ws?token=<jwt_access_token>")
    w("ws://localhost:16600/v1/ws?api_key=<api_key>")
    w("```")
    w()
    ...
    w()
    w("# 3. Verify")
    w("curl http://localhost:16600/health")
    w("curl http://localhost:16600/ready")
    w("```")
    w()
    w("### Run Modes")
    w()
