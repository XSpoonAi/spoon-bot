...
class TestAgentLoopStream:
    ...
    async def test_stream_yields_typed_dicts(self):
        ...
        # Mock LLMResponseChunk objects returned by ChatBot.astream()
        mock_chunk_1 = MagicMock()
        mock_chunk_1.delta = "Hello"

        mock_chunk_2 = MagicMock()
        mock_chunk_2.delta = " World"
        ...
        agent._chatbot = MagicMock()
        agent._chatbot.astream = mock_astream
    ...
    async def test_stream_handles_string_chunks(self):
        """stream() should handle delta strings from ChatBot.astream()."""
        from spoon_bot.agent.loop import AgentLoop

        # ChatBot.astream yields LLMResponseChunk with .delta
        mock_c1 = MagicMock()
        mock_c1.delta = "Hello"
        mock_c2 = MagicMock()
        mock_c2.delta = " World"
    ...
    async def test_stream_handles_thinking_chunks(self):
        """stream() wraps ChatBot.astream() deltas as content chunks.

        Note: thinking is stripped by AgentLoop.stream() and all chunks
        from ChatBot.astream() are yielded as ``type=content``.
        """
        from spoon_bot.agent.loop import AgentLoop

        c1 = MagicMock()
        c1.delta = "Let me think..."
        c2 = MagicMock()
        c2.delta = "Result"
        ...
        # All chunks are wrapped as content (thinking kwarg is stripped)
        assert len(content_chunks) == 2
        assert content_chunks[0]["delta"] == "Let me think..."
        assert content_chunks[1]["delta"] == "Result"
    ...
    async def test_stream_error_handling(self):
        ...
        async def mock_astream(messages, **kwargs):
            ...
            yield MagicMock(delta="never")  # pragma: no cover
    ...
    async def test_stream_skips_session_save_on_error(self):
        """stream() should emit done with error when ChatBot.astream() fails immediately."""
        from spoon_bot.agent.loop import AgentLoop
    ...
    async def test_stream_saves_session_on_success(self):
        """stream() should yield content and done chunks on success."""
        from spoon_bot.agent.loop import AgentLoop
